locals {
  ...
  env_vars_keys        = var.map_environment != null ? keys(var.map_environment) : var.environment_vars != null ? [for m in var.environment_vars : lookup(m, "name")] : []
  env_vars_values      = var.map_environment != null ? values(var.map_environment) : var.environment_vars != null ? [for m in var.environment_vars : lookup(m, "value")] : []
  ...
  # Commenting out this section as we can use this later if we are moving to Datadog later 
  ...
  container_definition = {
    name                   = var.name
    image                  = join(":", [var.container_image, var.environment])
